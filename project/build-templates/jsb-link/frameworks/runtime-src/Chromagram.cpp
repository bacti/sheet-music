    ...
    numBinsToSearch (2)
{
    ...
    chromagram.resize(CHROMAGRAM_SIZE);
...
void Chromagram::calculateChromagram()
{
    ...
    for (int n = 0; n < CHROMAGRAM_SIZE; n++)
    {
        float chromaSum = 0.0;
        
        for (int harmonic = 1; harmonic <= numHarmonics; harmonic++)
        {
            int centerBin = round ((noteFrequencies[n] * harmonic) / divisorRatio);
            ...
            chromaSum += (maxVal / (float) harmonic);
        }
