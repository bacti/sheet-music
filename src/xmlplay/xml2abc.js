...
(function () {  // all definitions inside an anonymous function
    ...
    function Note (duration, n) {
        this.time = 0;      // the time in XML division units
        this.duration = duration;     // duration of a note in XML divisions
    ...
    Music.prototype.incTime = function (dt) {
        this.time += dt;
        if (this.time < 0) this.time = 0;    // erroneous <backup> element
    ...
    Music.prototype.appendObj = function (v, obj, duration) {
        obj.time = this.time;
        this.voices [v].push (obj);
        this.incTime (duration);
        if (this.time > this.vtimes[v]) this.vtimes[v] = this.time;  // don't update for inserted earlier items
    }
    Music.prototype.appendElemT = function (v, elem, time) {    // insert element at specified time
        var obj = new Elem (elem);
        obj.time = time;
        this.voices [v].push (obj);
    }
    ...
    Music.prototype.appendNote = function (v, note, noot) {
        ...
        this.appendObj (v, note, parseInt (note.duration));
    ...
    Music.prototype.addBar = function (lbrk, m) {   // linebreak, measure data
        ...
        this.time = this.maxtime;           // the time of the bar lines inserted here
        for (var v in this.vnums) {
    ...
    window.ABCoutput = function(fnmext, pad, X, options) {
        this.fnmext = fnmext;
    ...
    function abcdur (nx, divs, uL) {    // convert an musicXML duration d to abc units with L:1/uL
        if (nx.duration == 0) return '';     // when called for elements without duration
        var num, den, numfac, denfac, dabc, t;
        t = simplify (uL * nx.duration, divs * 4); // L=1/8 -> uL = 8 units
        num = t[0]; den = t[1];
    ...
    function mkBroken (vs) {        // introduce broken rhythms (vs: one voice, one measure)
        ...
        while (i < vs.length - 1) {
            ...
            if (!n1.fact && !n2.fact && n1.duration > 0 && n2.beam) {    // skip if note in tuplet or has no duration or outside beam
                if (n1.duration * 3 == n2.duration) {
                    n2.duration = (2 * n2.duration) / 3;
                    n1.duration = n1.duration * 2;
                    n1.after = '<' + n1.after;
                    i += 1;             // do not chain broken rhythms
                } else if (n2.duration * 3 == n1.duration) {
    ...
    function sortMeasure (voice, m) {
        ...
        voice.sort (function (a, b) { return a.time - b.time || a.pos - b.pos; } )   // (stable) sort objects on time
        var time = 0;
        var v = [];
        var rs = [];                                // holds rests in between notes
        for (var i = 0; i < voice.length; ++i) {    // establish sequentiality
            var nx = voice [i];
            if (nx.time > time) {                   // fill hole with invisble rest
                v.push (new Note (nx.time - time, 'x'));
                rs.push (v.length - 1);
            }
            if (nx instanceof Elem) {
                if (nx.time < time) nx.time = time; // shift elems without duration to where they fit
                v.push (nx);
                time = nx.time;
                continue;
            }
            if (nx.time < time) {                   // overlapping element
                ...
                if (o.time <= nx.time) {            // we can do something
                    if (o.ns[0] == 'z') {           // shorten rest
                        o.duration = nx.time - o.time;
                        if (o.duration == 0) v.pop ();   // nothing left, remove note
                        ...
                        if (nx.duration <= 0) continue;           // nothing left
        ...
        while (xs.length) {
            ...
            for (i = 0; i < maten.length; ++i) { // all measures
                ...
                for (j = 0; j < m.length; ++j) {
                    ...
                    if ((e instanceof Elem) || e.duration == 0) continue;    // no real durations
    ...
    function perc2map (abcIn) {
        ...
        var abc = percSvg, ls, i, x, r, id='default', maps = {'default':[]};
        ls = abcIn.split ('\n');
    ...
    // parser
    //----------------
    window.Parser = function(options) {
    ...
    Parser.prototype.doNote = function ($n) {
        ...
        var duration = $n.find ('duration').text ();
        ...
        if (!duration || note.grace) duration = 0;
        ...
        note.duration = parseInt (duration);
    ...
    Parser.prototype.doAttr = function ($e) {
        ...
        first = this.msc.time == 0 && this.msr.ixm == 0;    // first attributes in first measure
        if (fifths) {
    ...
    Parser.prototype.doDirection = function ($e, i, $es) {  // parse a musicXML direction tag
        ...
        function addDirection (dit, x, vs, time, stfnum) {
            if (!x) return;
            vs = x.indexOf ('!8v') >= 0 ? dit.stfMap [stfnum] : [vs]; // ottava's go to all voices of staff
            vs.forEach (function (v) {
                if (time != null)       // insert at time of encounter
                    dit.msc.appendElemT (v, x.replace ('(',')').replace ('ped','ped-up'), time);
                else
        ...
        function startStop (dit, dtype, vs, stfnum) {
            ...
            if (type in typmap) {                   // opening the direction
                ...
                if (k in dit.dirStk) {             // closing direction already encountered
                    ...
                    if (sk.type == 'stop')
                        addDirection (dit, x, vs, sk.time, stfnum);
                    else {
                ...
                if (k in dit.dirStk) {             // matching open direction found
                    ...
                    dit.dirStk [k] = { type:'stop', time: dit.msc.time };
        ...
        if (tempo) {
            tempo = parseFloat (tempo).toFixed (0); // hope it is a number and insert in voice 1
            if (this.msc.time == 0 && this.msr.ixm == 0) { // first measure -> header
                abcOut.tempo = tempo;
...
export const XmlParser = (xmltree, options_parm) =>
{
    ...
    options.p = options.p ? options.p.split (',') : []

    window.abcOut = new ABCoutput('.abc', '', 0, options)
    const parser = new Parser(options)
    parser.parse(xmltree)
