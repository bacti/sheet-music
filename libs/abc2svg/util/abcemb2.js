...
// -- abc2svg init argument
    user = {
	OnError: function(msg, l, c) {	// get the errors
		errtxt += clean_txt(msg) + '\n'
	},
	OnLoad: function(str) {	// image output
		new_page += str
	},
...
function ready() {
	...
	// prepare for playing
	delete user.OnLoad;		// stop SVG generation
	old_gm = user.GetAbcModel;
	user.GetAbcModel = function(tsfirst, voice_tb, music_types, info) {
			if (old_gm)
