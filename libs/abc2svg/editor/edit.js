...
function render2() {
	...
	user.OnLoad = user.my_img_out;
	user.GetAbcModel = null;
...
function play_tune(what) {
	...
	if (!play.a_pe) {		// if no playing event
		user.OnLoad = null	// get the schema and stop SVG generation
		user.GetAbcModel = play.abcplay.add // inject the model in the play engine
